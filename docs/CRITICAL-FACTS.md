...
- **ONLY compare first 42 seconds** = 12,600 packets = 302,400 bytes
  ...
  - I suspect the reference/ cdg got its length from the dummy .wav file and we
    may chose to do the same; but its not importent for the moment so long as what we
   do render is junk.
...
- This is why rendering is wrong - the prelude is different
  No, its wrong because we made wrong assumptino and didn't implement the same
  packet genration and therefore created incomplete packet set and therefore
  missing critical frame update operations
