...
- Rate: 75 packets per second (packets/sec). Convert packets -> seconds with `seconds = packets / 75`.
...
-----------------------------------------------------------
- If a script or tool computes packet count using 96 instead of 24 (e.g. `packets = buf.length / 96`), it will report 1/4 the true number of packets. Because duration is computed as `packets / 75`, that yields a duration 4× too short.
...
2. Verify duration calculations use `packets / 75`.
...
node -e "const fs=require('fs');const p=fs.readFileSync(process.argv[1]);console.log('bytes',p.length,'packets',p.length/24,'seconds',p.length/24/75)" sample.cdg
```
