...
Usage:
  $ compare-cdg-stats.ts --run a.cdg b.cdg
...
*/
import fs from 'fs'
import { readScriptMd } from '../src/utils/bin-utils'
import { CDG_PACKET_SIZE } from '../src/cdg/constants'
...
if (!shouldRun)
{
  console.log('Usage: compare-cdg-stats.ts --run a.cdg b.cdg')
...
if (files.length !== 2)
{
  console.error('Usage: compare-cdg-stats.ts a.cdg b.cdg --run')
  process.exit(2)
}
const A = fs.readFileSync(files[0])
const B = fs.readFileSync(files[1])
function cmdAt(buf: Buffer, idx: number)
{
  return buf[idx * CDG_PACKET_SIZE + 1] & 0x3F
}
...
const indicesAemptyBtile: number[] = []
const indicesBemptyAtile: number[] = []
for (let i = 0; i < Math.min(pa, pb); i++)
{
  ...
  {
    countAemptyBtile++
    if (indicesAemptyBtile.length < 20) indicesAemptyBtile.push(i)
  }
  if (cb === 0 && ca === 6)
  {
    countBemptyAtile++
    if (indicesBemptyAtile.length < 20) indicesBemptyAtile.push(i)
  }
}
...
function totalTiles(buf: Buffer)
{
  let c = 0
