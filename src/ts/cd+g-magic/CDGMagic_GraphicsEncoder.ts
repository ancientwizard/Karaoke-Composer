...
import { getDefaultTransition, type TransitionData } from "@/ts/cd+g-magic/TransitionFileReader";
...
/**
 * BMP pixel and palette data for rendering
 */
...
export class CDGMagic_GraphicsEncoder {
   ...
   *
   * Mirrors C++ reference: CDGMagic_GraphicsEncoder::bmp_to_fonts()
   *
   ...
   *
   * @param bmpData BMP pixel data with palette
  ...
  ): CDGMagic_FontBlock[]
  {
    ...
    if (DEBUG) {
      console.debug(`[bmp_to_fonts] Screen dimensions: ${VRAM_WIDTH}×${VRAM_HEIGHT} (${SCREEN_TILES_WIDE}×${SCREEN_TILES_HIGH} blocks)`);
      ...
      console.debug(`[bmp_to_fonts] Processing ${trans_data.blocks.length} blocks in transition order`);
      
      // Show min/max Y coordinates
      const yCoords = trans_data.blocks.map(([_, y]) => y);
      const minY = Math.min(...yCoords);
      const maxY = Math.max(...yCoords);
      console.debug(`[bmp_to_fonts] Y coordinates range: ${minY}-${maxY}`);
    }
