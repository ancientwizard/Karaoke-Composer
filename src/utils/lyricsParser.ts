...
export function lyricLineToText(lyricLine: LyricLine): string {
  return lyricLine.words.map(word => {
...
export function parseLyricsText(lyricsText: string): LyricLine[] {
  ...
  return lines.map((line, index) => 
    parseLyricsLine(line.trim(), index + 1, `line-${Date.now()}-${index}`)
  )
}
...
function updateWordTiming(word: WordTiming): void {
  const timedSyllables = word.syllables.filter(syl => 
    syl.startTime !== undefined && syl.endTime !== undefined
  )
